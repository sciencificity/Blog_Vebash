...
- We'll do more advanced `dplyr` `r emo::ji("package")` workflows.
...
- `r emo::ji("question")` `r emo::ji("thinking")` Why didn't I also choose the `episode` column of <span style="color: #5196b4;background-color:#f4e4e7">results</span> as a join column? <br> `r emo::ji("check")` Yes, that column is not in `baker_results`, since `baker_results` contains data about how the baker did overall in the series they appeared, that is, one row per baker. <br>The `results` data however, contains info per baker, per episode, for the series they appeared in - i.e. if they flopped (were `OUT` `r emo::ji("wink")`), in the second episode of a series that contained 10 episodes, their name would appear 10 times in the `results` table, but their "result" value will be `NA` from episode 3 onwards.
...
I included the above to show that each time we "talk" to a table we must do so through our connection, because I often make the mistake of not including the `tbl(con, "name_of_tbl_i_am_joining")` in the __join__ function. I, more times than I care to admit `r emo::ji("exasperation")`, __incorrectly__ write:
...
I would like to help you, not repeat my mistake `r emo::ji("confused")`, so heads up <span style="color: #FFE4E1;background-color:#FD5800">AVOID THE FOLLOWING</span> `r emo::ji("stop")`:
...
How about that? Notice the __A tibble: 24 x 24__! R now, has the data in it's local environment, and can definitively tell us it knows there are 24 observations (no more _lazy query_) `r emo::ji("smile")`.
