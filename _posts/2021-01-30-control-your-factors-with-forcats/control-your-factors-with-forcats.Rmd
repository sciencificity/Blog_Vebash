...
# {forcats} `r emoji::emoji("package")`

The <span style="color: #5196b4;background-color:#f4e4e7">forcats</span>  `r emoji::emoji("package")` is meant to handle `factors` which is R's data type for categorical data. `forcats` is __for__ <b>cat</b>egorical data, and is an [anagram](https://r4ds.had.co.nz/factors.html) for `factors` `r emoji::emoji("cool")`. 
...
We're going to work with non-numeric data that may be treated as factors in this post, and learn how to use the forcats `r emoji::emoji("package")` to make that task easier for us.

## Data

We're going to use the data from the awesome [TidyTuesday Project](https://github.com/rfordatascience/tidytuesday) `r emoji::emoji("sparkles")`:
