...
You know R, especially the `dplyr` `r emo::ji("package")`. Even though the `dplyr` `r emo::ji("package")` is so well written to mimic the SQL syntax - __select()__, __group_by()__, __left_join()__ etc. there is still a cognitive load when you switch between using R syntax, and SQL syntax (ask me, who has often written `==` in SQL syntax on Athena only to wonder why I am getting an error `r emo::ji("silence")`).
...
In this tutorial you will learn how to use `dbplyr`, which is a database back-end of `dplyr`, to execute queries directly in your RDBMS all the while writing R tidyverse syntax `r emo::ji("wow")` `r emo::ji("star")`.
...
- dbplyr: The `r emo::ji("package")` we may use to talk to our database, all the while using `dplyr` syntax in our code.<br>`install.packages("dbplyr")`
...
- Other DBI compliant R packages you may need for your use case - e.g. `RPostgres` is for connecting to a PostgreSQL RDBMS. *Note: Not used in this tutorial (brief overview in slides), and [RStudio's DB Website](https://db.rstudio.com/) is a treasure trove of information* `r emo::ji("technologist")`.
...
Ok, let's flex our `dplyr` skills `r emo::ji("smile")`.
