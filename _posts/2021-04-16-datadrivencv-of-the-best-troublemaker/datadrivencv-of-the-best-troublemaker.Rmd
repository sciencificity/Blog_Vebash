...
public domain, so I thought I would create a <span style="color: #5196b4;background-color:#f4e4e7"><bold>mock CV</bold></span> for the best "troublemaker" to ever have graced this earth - at least in my opinion - Nelson Rolihlahla Mandela `r emo::ji("excited")`!
...
We're going to run through the motions of setting up a [CV](https://sciencificity.github.io/madiba-datadriven-cv/) using the <span style="color: #5196b4;background-color:#f4e4e7">{datadrivencv} </span>`r emo::ji("package")`.

## Download {datadrivencv} `r emo::ji("package")`
...
The Google Sheets method is better because every amendment you make on the Excel workbook will mean re-running this code `r emo::ji("fear")`!
